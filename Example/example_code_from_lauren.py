...
np.random.seed(42)
spectral_axis = np.linspace(11.0, 1.0, 200) * u.GHz

spectral_model = models.Gaussian1D(
    amplitude=5 * (2 * np.pi * 0.8**2) ** -0.5 * u.Jy,
    mean=5 * u.GHz,
    stddev=0.8 * u.GHz,
)
flux = spectral_model(spectral_axis)
flux += np.random.normal(0.0, 0.05, spectral_axis.shape) * u.Jy
uncertainty = StdDevUncertainty(0.2 * np.ones(flux.shape) * u.Jy)
noisy_gaussian = Spectrum1D(
    spectral_axis=spectral_axis, flux=flux, uncertainty=uncertainty
)

size = 200
spec_axis = np.linspace(4500.0, 6500.0, num=size) * u.AA

rest_value = 6000.0 * u.AA  # ? How did they get these values?
...
# 1
def extract_spec(
    data1, spectral_axis1, spec_flux1, uncertainty, size, spec_axis, rest_value
):
    mean1 = 5035.0 * u.AA  # ? How did they get these values?
    f1 = np.random.randn(size) * 0.5 * u.Jy
    g1 = models.Gaussian1D(amplitude=30 * u.Jy, mean=mean1, stddev=10.0 * u.AA)
    ...
    ospec1 = Spectrum1D(
        spectral_axis=spectral_axis1,
        flux=spec_flux1,
        uncertainty=spec_uncertainty1,
        velocity_convention="optical",
        rest_value=rest_value,
    )
...
# 2
mean2 = 5015.0 * u.AA  # ? How did they get these values?
f2 = np.random.randn(size) * 0.5 * u.Jy
g2 = models.Gaussian1D(amplitude=30 * u.Jy, mean=mean2, stddev=10.0 * u.AA)
...
tspec2 = Spectrum1D(
    spectral_axis=spectral_axis2, flux=spec_flux2, uncertainty=spec_uncertainty2
)
